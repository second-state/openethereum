...
impl<B: Backend> State<B> {
	...
	pub fn from_existing(mkvs: Box<dyn MKVS>, db: B, root: H256, account_start_nonce: U256, factories: Factories) -> TrieResult<State<B>> {
		let state = State {
			mkvs,
			db,
			root,
	...
	#[must_use]
	fn update_account_cache(require: RequireCache, account: &mut Account, state_db: &B, mkvs: &(dyn MKVS)) -> bool {
